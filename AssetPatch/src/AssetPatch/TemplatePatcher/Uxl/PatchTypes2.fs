...
module PatchTypes2 =
    ...
    type FlocClassification = 
        ...
        member x.ToAssocs() = 
            [ ("Functional Location",           x.FunctionalLocation.ToString())
            ; ("Deletion Ind",                  if x.ClassDeletionInd then "X" else "")
            ; ("Class Type",                    "003")
            ; ("Class",                         x.Class)
            ; ("Status",                        x.Status)
            ; ("Characteristics",               x.CharacteristicName)
            ; ("Char Value",                    x.CharacteristicValue)
            ; ("Ch.Deletion Ind.",              if x.CharDeletionInd then "X" else "")
            ] |> AssocList.ofList
    ...
    type EquiClassification = 
        { EquipmentId: EquipmentId
          ClassDeleteInd: bool option
          Class: string
          Status: string
          CharacteristicName: string
          CharacteristicValue: string
          CharDeleteInd: bool option
        }
        member x.ToAssocs() = 
            [ ("Equipment",                     x.EquipmentId)
            ; ("Delete Ind.",                   optionalBool x.ClassDeleteInd)
            ; ("Class Type",                    "002")
            ; ("Class",                         x.Class)
            ; ("Status",                        x.Status)
            ; ("Characteristics",               x.CharacteristicName)
            ; ("Char Value",                    x.CharacteristicValue)  
            ; ("Ch. Delete Ind.",               optionalBool x.CharDeleteInd)
            ] |> AssocList.ofList
